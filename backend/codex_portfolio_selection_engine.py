...
class PortfolioSelectionEngine:
    ...
    def select_best(
        ...
        if len(strategies) <= portfolio_size:
            logger.info(f"[CODEX PORTFOLIO SELECTION] All {len(strategies)} strategies selected")
            return {
        ...
        logger.info(f"[CODEX PORTFOLIO SELECTION] Selected {len(selected)} strategies")
