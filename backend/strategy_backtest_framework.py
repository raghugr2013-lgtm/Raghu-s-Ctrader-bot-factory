...
class BacktestConfig:
    ...
    max_position_size: float = 0.1  # Maximum lot size cap
...
class SimpleBacktester:
    ...
    def __init__(self, data: pd.DataFrame, config: BacktestConfig, symbol: str):
        ...
        if 'JPY' in symbol:
            self.pip_value = 0.01  # JPY pairs
            self.pip_size = 0.01
            self.point_value_per_lot = 1000  # $1000 per lot per 1.0 move
        elif 'XAU' in symbol or 'GOLD' in symbol:
            # XAUUSD: 1 lot = 100 oz
            # 1.0 price move = $100 per lot
            # 0.01 move = $1 per lot
            self.pip_value = 1.0  # Gold point
            self.pip_size = 1.0
            self.point_value_per_lot = 100  # $100 per lot per 1.0 move
        else:
            # Standard forex (EURUSD, GBPUSD, etc)
            # 1 lot = 100,000 units
            # 1 pip (0.0001) = $10 per lot
            self.pip_value = 0.0001
            self.pip_size = 0.0001
            self.point_value_per_lot = 100000  # $10 per lot per 0.0001 move
        
        logger.info(f"Instrument config for {symbol}:")
        logger.info(f"  Pip size: {self.pip_size}")
        logger.info(f"  Point value per lot: ${self.point_value_per_lot} per {self.pip_size} move")
    
    def calculate_position_size(self, stop_loss_pips: float, current_balance: float = None) -> float:
        """
        Calculate position size based on risk percentage
        
        Args:
            stop_loss_pips: Stop loss distance in pips/points
            current_balance: Current account balance (uses initial if None)
        ...
        # Calculate pip value in dollars per lot
        if 'XAU' in self.symbol or 'GOLD' in self.symbol:
            # For gold: 1 point = $100 per lot
            # So stop loss of X points = $100 * X per lot
            stop_loss_value_per_lot = stop_loss_pips * 100.0
        else:
            # For forex: 1 pip (0.0001) = $10 per lot
            stop_loss_value_per_lot = stop_loss_pips * 10.0
    ...
    def execute_backtest(self, signals_df: pd.DataFrame, strategy_name: str) -> BacktestResult:
        """
        Execute backtest based on signals with dynamic position sizing
        ...
        for i in range(len(df)):
            ...
            if df.iloc[i]['position'] != 0 and current_position is None:
                ...
                if 'XAU' in self.symbol or 'GOLD' in self.symbol:
                    # Gold: use smaller ATR multiplier (more conservative)
                    stop_loss_distance = atr * 1.5  # 1.5x ATR
                    # Also cap the stop loss at reasonable level
                    max_sl = 50.0  # Max 50 points for gold
                    stop_loss_distance = min(stop_loss_distance, max_sl)
                else:
                    # Forex: convert ATR to pips
                    stop_loss_distance = (atr / self.pip_size) * 2.0  # 2x ATR in pips
                    # Cap at 100 pips
                ...
                # Calculate adjusted entry price with spread
                entry_price = df.iloc[i]['close']
                if df.iloc[i]['position'] > 0:  # Long
            ...
            # Exit signal or stop loss hit
                ...
                # Max holding period (48 hours for H1 = 48 candles)
                if i - current_position['entry_idx'] >= 48:
                    should_exit = True
                    exit_reason = "timeout"
                ...
                if should_exit:
                    ...
                    # Calculate P&L
                    if 'XAU' in self.symbol or 'GOLD' in self.symbol:
                        # Gold: price difference * $1 per 0.01 move * position size
                        price_diff = exit_price - current_position['entry_price']
                        if current_position['direction'] == 'short':
                            price_diff = -price_diff
                        profit_usd = price_diff * current_position['position_size'] * 100  # 100 oz per lot
                        profit_pips = price_diff  # In gold points
                    else:
                        ...
                        # Convert to USD: pips * $10 per pip per lot * position size
