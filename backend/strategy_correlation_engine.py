...
class CorrelationEngine:
    ...
    def filter_correlated(
        ...
        logger.info(f"[CORRELATION ENGINE] Filtered {len(strategies)} → {len(filtered)}")
        logger.info(f"[CORRELATION ENGINE] Avg Correlation: {avg_corr:.3f}")
        logger.info(f"[CORRELATION ENGINE] Removed {len(removed)} correlated strategies")
